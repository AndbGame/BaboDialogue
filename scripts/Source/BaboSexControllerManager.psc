...
Function Surrender();WIP
	if PlayerRef.isinfaction(BaboPlayerFaction)
		PlayerRef.removefromfaction(BaboPlayerFaction)
...
Event AnimalCareEventMarkarthAfterSex(string eventName, string argString, float argNum, form sender)
	;CorruptionExp(2.0)
	BaboSpectatorWOEnemyScript.stop()
	BaboEventAnimalCareMarkarth.setstage(70)
EndEvent

Function CustomMFGPain(actor akactor, int iindex)
	if iindex == 0
		(BaboMonitorScript as BaboDiaMonitorScript).Backtoinitial(akactor)
	else
		(BaboMonitorScript as BaboDiaMonitorScript).PainMenu(akactor, iindex)
	endif
EndFunction
...
int Function RollRapeChance(Actor akRef, int diminishchance)
		...
		int CharacterRank = akRef.getfactionrank(SLAX_AggressiveFaction) as int
